...
export class FicheEmployer extends React.Component {
  ...
  async componentDidMount() {
    try {
      ...
      const response = await fetch(
        'http://localhost:3001/employers/' + this.props.match.params.id
      );
  ...
  async modifierEmployer(id, nom, prenom, fonction, photo, departement) {
    try {
      const response = await fetch(
        'http://localhost:3001/employers/' + this.state.pokemonID,
        {
          method: 'PUT',
  ...
  handleEdit(event) {
    ...
    const id = this.state.donneesRecues.id;
    ...
    this.modifierEmployer(id, nom, prenom, fonction, photo, departement);
  }
          ...
          <Col lg='10'>
            <Form>
              <Form.Group controlId='nomEmployer'>
                ...
                <Form.Control
                  type='text'
                  defaultValue={this.state.donneesRecues.Nom}
                />
              </Form.Group>
              <Form.Group controlId='prenomEmployer'>
                ...
                <Form.Control
                  type='text'
                  defaultValue={this.state.donneesRecues.Prenom}
                />
              </Form.Group>
              <Form.Group controlId='fonctionEmployer'>
                ...
                <Form.Control
                  type='text'
                  defaultValue={this.state.donneesRecues.Fonction}
                />
              </Form.Group>
              ...
              <Form.Group controlId='departementEmployer'>
                ...
                <Form.Control
                  type='text'
                  defaultValue={this.state.donneesRecues.Departement}
                />
              </Form.Group>
            ...
            <p className='btn btn-danger mt-5' onClick={this.handleDelete}>
